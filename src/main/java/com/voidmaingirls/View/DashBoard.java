...
public class DashBoard {
    Scene dashboardScene,SignUpScene,weekScene,caloricScene,recipeScene,drinkupScene,userProfileScene,botScene;
    ...
    public VBox createScene(Runnable back) {
        // Implementation for creating the dashboard scene
        // This will include setting up the layout, components, and styles

        

        ImageView userIcon = new ImageView(new Image("/Assets/Images/icon.png"));
        userIcon.setFitHeight(70);
        userIcon.setFitWidth(70);

        VBox icon = new VBox(0,userIcon);
        icon.setLayoutX(1200);
        icon.setLayoutY(10);


        
        icon.setOnMouseClicked(e -> {
            openUserProfileFromDashboard();
            });
        ...
        weekBite.setOnMouseClicked(new EventHandler<Event>() {
            ...
            public void handle(Event arg0) {
                initializeWeekBites();
                dashboardStage.setScene(weekScene);
        ...
        recipeHub.setOnMouseClicked(new EventHandler<Event>() {
            ...
            public void handle(Event arg0) {
                initializeRecipehub();
                dashboardStage.setScene(recipeScene);
            }
        ...
        calorieVault.setOnMouseClicked(new EventHandler<Event>() {
            ...
            public void handle(Event arg0) {
                initializeCalori();
                dashboardStage.setScene(caloricScene);
        ...
        drinkUp.setOnMouseClicked(new EventHandler<Event>() {
            ...
            public void handle(Event arg0) {
                initializeDrinkup();
                dashboardStage.setScene(drinkupScene);
            }
        ...
        header.setPadding(new Insets(0,140,0,0));
...
Label description = new Label(
    ...
    "Special thanks to Core2Web and our mentor Shashi Sir\n" +
    "for their constant guidance and support throughout this journey."
);
...
aboutSection.setPrefWidth(1400);
aboutSection.setLayoutY(2750); // adjust as needed

        Pane root = new Pane();

        Region backgroundBox = new Region();
        backgroundBox.setPrefSize(1300, 3300);
        ...
        StackPane mainRoot1 = new StackPane();
    ...
    menuContainer.getChildren().forEach(section -> {
        ...
        mainRoot1.getChildren().addAll(scroll);
...
chatBotBtn.setOnAction(e -> {
    initializeChatbot();
    dashboardStage.setScene(botScene);
});
...
VBox mainRoot = new VBox(scroll);
return mainRoot;
    }

    public void handleBack() {
    dashboardStage.setScene(dashboardScene);  // return to home scene
    }

    private void initializeWeekBites(){
        WeekBites s1 = new WeekBites();
        s1.setWeekStage(dashboardStage);
    ...
    private void initializeCalori(){
        ...
        s1.setCalorieStage(dashboardStage);
    ...
    private void initializeRecipehub(){
        ...
        s1.setRecipeStage(dashboardStage);
    ...
    private void initializeDrinkup(){
        ...
        s1.setDrinkupStage(dashboardStage);
        drinkupScene= new Scene(s1.createScene(this:: handleBack),1300,700);
        s1.setDrinkupScene(drinkupScene);
    }

    private Label createStatCard(String emoji, String number, String label) {
    Label statLabel = new Label(emoji + "\n" + number + "\n" + label);
    statLabel.setTextAlignment(TextAlignment.CENTER);
    statLabel.setTextFill(Color.web("#2E7D32"));
    statLabel.setPadding(new Insets(15));
    statLabel.setStyle(
        "-fx-background-color: #f4f8e4ff;" +
    ...
    statLabel.setFont(Font.font("Poppins", FontWeight.BOLD, 20));
    statLabel.setPadding(new Insets(20));
    statLabel.setMinWidth(180);
    return statLabel;
...
private void animateSwap(HBox hbox, Node first, Node second) {
    // Slide out animation
    TranslateTransition slideOutFirst = new TranslateTransition(Duration.millis(400), first);
    slideOutFirst.setToX(200);

    TranslateTransition slideOutSecond = new TranslateTransition(Duration.millis(400), second);
    slideOutSecond.setToX(-200);

    FadeTransition fadeOutFirst = new FadeTransition(Duration.millis(400), first);
    fadeOutFirst.setToValue(0);

    FadeTransition fadeOutSecond = new FadeTransition(Duration.millis(400), second);
    fadeOutSecond.setToValue(0);

    ParallelTransition outAnim = new ParallelTransition(slideOutFirst, slideOutSecond, fadeOutFirst, fadeOutSecond);
    outAnim.setOnFinished(e -> {
        hbox.getChildren().setAll(second, first);

        // Reset position for slide in
        first.setTranslateX(-200);
        second.setTranslateX(200);

        // Slide in animation
        TranslateTransition slideInFirst = new TranslateTransition(Duration.millis(400), first);
        slideInFirst.setToX(0);

        TranslateTransition slideInSecond = new TranslateTransition(Duration.millis(400), second);
        slideInSecond.setToX(0);

        FadeTransition fadeInFirst = new FadeTransition(Duration.millis(400), first);
        fadeInFirst.setToValue(1);

        FadeTransition fadeInSecond = new FadeTransition(Duration.millis(400), second);
        fadeInSecond.setToValue(1);

        new ParallelTransition(slideInFirst, slideInSecond, fadeInFirst, fadeInSecond).play();
    });

    outAnim.play();
}

private void openUserProfileFromDashboard() {
    try {
        // 🔹 Fetch profile data from Firestore
        Map<String, Object> data = UserProfileController.getCompleteUserProfile();

        // 🔹 Convert to Map<String, String>
        Map<String, String> userData = new HashMap<>();
        data.forEach((k, v) -> userData.put(k, String.valueOf(v)));

        // 🔹 Create view and back action
        ...
        Runnable backToHome = () -> {
            try {
                ...
                dashboardStage.close(); // or stage.hide()
            } catch (Exception e) {
                e.printStackTrace();
            }
        };

        // 🔹 Load scene and switch
        Scene userProfileScene = new Scene(profile.createScene(backToHome, userData), 1300, 700);
        profile.setUserStage(dashboardStage);
        profile.setUserScene(userProfileScene);
        dashboardStage.setScene(userProfileScene);
...
private void handleBackFromProfile() {
    System.out.println("Back to Dashboard triggered");
    dashboardStage.setScene(dashboardScene); // Make sure dashboardScene is already initialized
}
